...
describe Watir::Wait do
  describe "#until" do
    ...
    it "times out" do
      expect { Wait.until(0.5) { false } }.to raise_error(Wait::TimeoutError)
    end
  ...
  describe "#while" do
    ...
    it "times out" do
      expect { Wait.while(0.5) { true } }.to raise_error(Wait::TimeoutError)
    end
...
describe Watir::Element do
  ...
  describe "#when_present" do
    ...
    it "times out when not given a block" do
      expect { browser.div(id: 'bar').when_present(1).click }.to raise_error(Wait::TimeoutError,
  ...
  describe "#when_enabled" do
    ...
    it "times out when not given a block" do
      expect { browser.button(id: 'btn').when_enabled(1).click }.to raise_error(Wait::TimeoutError,
  ...
  describe "#wait_until_present" do
    ...
    it "times out if the element doesn't appear" do
      expect { browser.div(id: 'bar').wait_until_present(1) }.to raise_error(Wait::TimeoutError,
        /^timed out after 1 seconds, waiting for (\{:id=>"bar", :tag_name=>"div"\}|\{:tag_name=>"div", :id=>"bar"\}) to become present$/
      )
    end
  end
